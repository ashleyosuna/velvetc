...
def concatenate_path(path: List[str], hash_length):
    result: str = path[0]

    for seq in path[1:]:
        overlap_len = max_overlap(result, seq)
        # result += seq[hash_length - 1:]
...
def tourbus(graph):
    ...
    while pq:
        ...
        for dest_id, mult in node_obj.out_edges.items():
            dest_node = graph.nodes[dest_id]

            edge_weight = calculate_edge_weight(dest_node, mult)
            ...
            if not discovered[dest_id]:
                ...
                if path_a == [] or path_b == []: # not a bubble
                    continue
                ...
                path_a_str = concatenate_path(path_a_seqs, graph.k)
                path_b_str = concatenate_path(path_b_seqs, graph.k)
               
                similar = compare_sequences(path_a_str, path_b_str)
